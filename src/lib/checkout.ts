...
import { isBulky } from "@/data/catalog";
...
export async function createOrder(input: CheckoutInput, siteUrl: string) {
  ...
  const items: OrderItem[] = input.items.flatMap((i) => {
    ...
    return p ? [{ slug: p.slug, name: p.name, brand: p.brand, price: Number(p.price), qty: Math.max(1, i.qty), image: (p.images as { url: string }[])?.[0]?.url, bulky: isBulky(p.category as string) }] : [];
  });
  ...
  const hasBulky = products.some((p) => isBulky(p.category as string));
  if (hasBulky && !sm.bulky) throw new Error("Bulky goods need store collection, our own delivery or pallet freight");
