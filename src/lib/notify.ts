...
const from = process.env.NOTIFY_FROM ?? "Elektro Dvořák <eshop@elektrodvorak.cz>";
...
export async function notifyOrderCreated(o: Order, staffEmail?: string | null) {
  ...
  const html = `<div style="font-family:Inter,Arial,sans-serif;max-width:560px">
    ...
    <p style="color:#666">Elektro Dvořák · Dvořák a synové, s.r.o. · Nádraží 604, Golčův Jeníkov · +420 702 521 521 · eshop@elektrodvorak.cz</p></div>`;
  await send(o.customer_email, `${cs ? "Objednávka" : "Order"} #${o.number} · Elektro Dvořák`, html);
