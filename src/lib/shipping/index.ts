...
export const carriers: Record<string, Carrier> = {
  ...
  store: {
    id: "store", label: "Osobní odběr na prodejně", configured: true, capability: "manual",
    async createShipment(order) { return { trackingNumber: `ED-${order.number}` }; },
    trackingUrl: () => "",
  },
  delivery: {
    id: "delivery", label: "Vlastní doprava a zapojení", configured: true, capability: "manual",
    async createShipment(order) { return { trackingNumber: `ED-${order.number}` }; },
    trackingUrl: () => "",
  },
};
