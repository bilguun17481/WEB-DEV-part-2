  ...
  return (
    <footer className="mt-24 bg-charcoal text-paper">
      ...
      <div className="container-x grid gap-10 py-14 md:grid-cols-[1.4fr_1fr_1fr_1fr]">
        <div>
          <div className="text-[20px] font-extrabold uppercase tracking-[-0.03em]">{store?.name ?? company.name}</div>
          <p className="mt-4 text-[13px] leading-relaxed text-neutral-400">
            {store?.legal ?? company.legal}<br />{store?.address ?? company.address}<br />{store?.phone ?? company.phone}<br />{store?.email ?? company.email}
          </p>
          <div className="mt-5 flex gap-3 text-neutral-400">
            {["facebook", "instagram", "youtube"].map((s) => (
              <a key={s} href={s === "facebook" ? "https://www.facebook.com/www.elektrodvorak.cz/" : "#"} aria-label={s} className="flex h-9 w-9 items-center justify-center border border-neutral-700 hover:border-paper hover:text-paper">
        ...
        <Col title={t(f.sales)} items={categories.map((c) => [`/${c.slug}/`, t(c.label)])} />
        <Col title={t(f.help)} items={[
          ["/servis/", t(dict.nav.servis)], ["/servis/", t(f.warranty)], ["#", t(f.delivery)], ["#", t(f.financing)], ["/servis/", t(f.parts)],
        ]} />
        <Col title={t(f.company)} items={[
          ["/prodejny/", t(f.stores)], ["/kontakt/", t(dict.nav.kontakt)], ["#", t(f.about)], ["#", t(f.terms)], ["#", t(f.privacy)],
        ]} />
      </div>
      <div className="border-t border-neutral-800">
        <div className="container-x flex flex-wrap items-center justify-between gap-3 py-5 text-[11px] text-neutral-500">
          <span>© {new Date().getFullYear()} {t(f.rights)}</span>
          <span className="uppercase tracking-[0.14em]">{brands.slice(0, 6).join(" · ")}</span>
        </div>
      </div>
    </footer>
  );
