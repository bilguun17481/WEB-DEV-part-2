          ...
          case "categories":
            return (
              <section key={s.id} className="container-x py-16">
                ...
                <div className={`mt-10 grid grid-cols-2 gap-4 ${s.categories.length % 3 === 0 && s.categories.length % 4 !== 0 ? "lg:grid-cols-3" : "lg:grid-cols-4"}`}>
            ...
            return (
              <section key={s.id} className="container-x py-16">
                <div className="flex items-end justify-between border-b hairline pb-5">
                  ...
                  <Link href={s.category ? `/${s.category}/` : "/velke-spotrebice/"} className="btn-link">{t(dict.home.all)} <span aria-hidden>→</span></Link>
                </div>
