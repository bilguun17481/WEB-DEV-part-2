...
export function CheckoutView({ shipping, payments, live, packetaKey }: { shipping: ShippingMethod[]; payments: PaymentMethod[]; live: boolean; packetaKey: string }) {
  ...
  const methods = shipping.filter((m) => !hasBulky || m.bulky);
  ...
  const needsAddress = Boolean(sm && !sm.needs_pickup_point && sm.carrier !== "store");
  ...
  if (hydrated && items.length === 0) return (
    <section className="container-x py-16 text-center"><p className="text-mute">{t(dict.cart.empty)}</p><Link href="/male-spotrebice/" className="btn-ink mt-6">{t(dict.cart.continue)}</Link></section>
  );
      ...
      <form className="mt-10 grid gap-12 lg:grid-cols-[7fr_4fr]" onSubmit={submit}>
        <div className="space-y-12">
          ...
          <fieldset>
            ...
            {hasBulky && <p className="mt-3 text-[12px] text-mute">{t(k.bulkyHint)}</p>}
