...
function Shipping() {
  ...
  const carriers = ["store", "delivery", "packeta", "ppl", "dpd", "ceska_posta", "gls", "fofr"];
  const blank: ShippingMethod = { id: "", carrier: "packeta", name: { cs: "", en: "" }, description: { cs: "", en: "" }, price: 0, free_over: null, enabled: true, needs_pickup_point: false, bulky: false, sort: (data?.length ?? 0) + 1 };
  ...
  const Form = ({ m, onChange }: { m: ShippingMethod; onChange: (m: ShippingMethod) => void }) => (
    <div className="grid gap-3 sm:grid-cols-2">
      ...
      <div className="flex flex-wrap gap-5 sm:col-span-2">
        ...
        <Toggle checked={m.bulky} onChange={(v) => onChange({ ...m, bulky: v })} label={t(s.bulky)} />
      </div>
    </div>
  );
  return (
