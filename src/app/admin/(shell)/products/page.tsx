...
export default function Products() {
  ...
  const bulk = async (s: "active" | "archived") => { for (const id of sel) { const p = list.find((x) => x.id === id); if (p) await repo().products.save({ ...p, status: s }); } setSel(new Set()); await refreshStorefront(["/", ...categories.map((c) => `/${c.slug}/`)]); toast(t(adm.common.saved)); reload(); };
