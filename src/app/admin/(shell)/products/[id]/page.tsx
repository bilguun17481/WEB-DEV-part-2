...
const blank: ShopProduct = { slug: "", brand: "Samsung", category: "televize-audio", name: "", price: 0, energy: "—", short: { cs: "", en: "" }, description: { cs: "", en: "" }, specs: [], colors: ["#1f1f1f"], tags: [], status: "draft", stock: 0, sku: "", images: [], videos: [], featured: false };
        ...
        <div className="space-y-4">
          <Card title={t(adm.products.basics)}>
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <Field label={t(adm.products.energy)}><Select value={p.energy} onChange={(e) => set({ energy: e.target.value as ShopProduct["energy"] })}>{["—", "A", "B", "C", "D", "E", "F", "G"].map((h) => <option key={h}>{h}</option>)}</Select></Field>
              <Field label={t(adm.products.size)}><Input value={p.size ?? ""} onChange={(e) => set({ size: e.target.value })} placeholder="55&quot; · 138 cm" /></Field>
              <Field label={t(adm.products.power)}><Input value={p.power ?? ""} onChange={(e) => set({ power: e.target.value })} placeholder="1 800 W" /></Field>
            </div>
