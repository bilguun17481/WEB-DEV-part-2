        ...
        <div className="space-y-4">
          <Card title={t(adm.orders.items)} actions={<div className="flex gap-2"><Badge tone={tone(o.payment_status)}>{t(adm.orders.ps[o.payment_status])}</Badge><Badge tone={tone(o.status)}>{t(adm.orders.st[o.status])}</Badge></div>}>
            <ul className="divide-y divide-hair text-[13px]">
              {o.items.map((i) => <li key={i.slug} className="flex items-center gap-3 py-2.5">{i.image ? <SmartImg src={i.image} size="thumb" className="h-10 w-10 rounded object-cover" /> : <div className="h-10 w-10 rounded bg-tile" />}<div className="flex-1"><div className="font-medium">{i.name}</div><div className="text-[11px] text-mute">{i.brand} · {i.slug}</div></div><div className="text-mute">{i.qty} × {money(i.price)}</div><div className="w-24 text-right font-semibold">{money(i.price * i.qty)}</div></li>)}
            </ul>
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <div>
                ...
                <div className="mt-2 flex flex-wrap gap-2">
                  ...
                  {o.shipping_carrier && !["store", "delivery"].includes(o.shipping_carrier) && <Button disabled={busy} onClick={label}>{t(adm.orders.label)}</Button>}
                </div>
