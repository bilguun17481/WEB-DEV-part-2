...
export async function generateMetadata({ params }: { params: Promise<{ category: string }> }): Promise<Metadata> {
  ...
  if (!page) return {};
  return { title: `${page.seo?.title?.cs || page.title.cs} · Elektro Dvořák`, description: page.seo?.description?.cs };
}

/** Serves both catalog categories (/televize-audio/) and pages built in the admin (/akce/, /o-nas/, …). */
