 ...
 * The shop has thousands of pages, so the crawl is capped (MAX_PAGES) and
 * limited to category and product paths; raise the cap or add OVERRIDES for
 * products the crawl misses.
 */
...
/** Catalog slug -> exact elektrodvorak.cz path, when name matching is not enough. */
const OVERRIDES = {
  // "lg-oled65b4": "/lg-oled65b4/prod12345.html",
};

const catalog = await loadCatalog();
const UA = { "user-agent": "Mozilla/5.0 (compatible; elektrodvorak-mockup-fetch/1.0)" };
...
// 1. Crawl category (katNNN.html) and product (prodNNN.html) pages, breadth-first, same host.
const seen = new Set([ROOT + "/"]);
const queue = [ROOT + "/"];
const pages = [];
const wanted = (p) => /\/kat\d+\.html$/.test(p) || /\/prod\d+\.html$/.test(p) || /\/[a-z0-9-]+\/?$/.test(p);
while (queue.length && pages.length < MAX_PAGES) {
  ...
  for (const m of body.matchAll(/href="((?:https?:\/\/www\.elektrodvorak\.cz)?\/[^"#?]*)"/g)) {
    const next = m[1].startsWith("http") ? m[1] : ROOT + m[1];
    if (!seen.has(next) && wanted(new URL(next).pathname)) { seen.add(next); queue.push(next); }
  }
}
...
const candidates = pages.map(({ url, body }) => {
  const title = (body.match(/<h1[^>]*>([\s\S]*?)<\/h1>/i)?.[1] ?? body.match(/<title>([^<]*)<\/title>/i)?.[1] ?? "")
    .replace(/<[^>]+>/g, "").replace(/\s*[|·-]\s*Elektro Dvořák.*$/i, "").trim();
  const og = body.match(/property="og:image"\s+content="([^"]+)"/i)?.[1];
  const imgs = [...body.matchAll(/<img[^>]+src="([^"]+)"/gi)].map((m) => m[1])
    .filter((s) => /\.(png|jpe?g|webp)(\?|$)/i.test(s) && !/logo|icon|flag|banner|bg_|pattern|heureka|zbozi/i.test(s));
